...
##########################################################
PATH_TO_OBJECT_DETECTION_REPO = "C://Users//Shanmukha Yenneti//Desktop//models//research//object_detection//" 
...
class ObjectDetectionPredict():
    ...
    def predict_single_image(self, image_file_path):
        ...
        """
        image = Image.fromarray(image_file_path)
        ...
        if correct_prediction:
            ...
            c=list(boxes)
            l=len(c)
            #for s, b, c in correct_prediction: #to draw for every person
                #print(b)
                #c=list(b)
                
            draw_rectangle(draw, c[0], 'green', 5) #first person
            #draw_rectangle(draw, c[l-1], 'red', 5) #last person
        else:
...
all_test_images = glob.glob("%s/*.png"%(test_image_dir.rstrip('/')))

cap = cv2.VideoCapture('C://Users//Shanmukha Yenneti//Desktop//race-7.mp4')

while True:
  ret, frame = cap.read()
  frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
  #### boxes are in [ymin. xmin. ymax, xmax] format
  scores, classes, img, boxes = prediction_class.predict_single_image(frame)
